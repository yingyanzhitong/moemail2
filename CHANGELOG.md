...
- **桌面授权筛选**：授权管理列表默认仅展示有效授权，可切换查看待激活、已过期、额度用尽、已停止或全部类型。
- **压缩批次明细**：压缩情况支持点击查看每次客户端回传的完成时间、成功张数、压缩前后 KB 大小、压缩比与软件版本。
- **飞书下载指南同步**：明确桌面端 Release 成功后必须更新飞书安装指南中的版本、发行页和三平台安装包下载链接，并验证匿名可访问。

### Tests

- **授权管理回归**：桌面授权后端 19 项测试与网页端生产构建通过。

## [1.14.48] - 2026-07-28

### Changed

- **桌面端一键更新**：发现新版本后，点击标题栏“更新”即直接下载、签名校验、安装并重启，不再弹出二次确认版本的窗口。
- **Auth Link 买家标识**：创建桌面端 Auth Link 接口支持可选 `buyer_id`；买家 ID 会持久化在授权记录，并展示在后台授权表格中。
- **压缩使用量回传**：客户端按本次执行批次回传原图字节、压缩后字节和软件版本；后台授权表格展示本次压缩比、累计压缩比和最近回传的软件版本。
- **桌面端版本**：TinyPNG 压缩助手升级至 `0.2.13`。

### Database

- **桌面授权指标**：新增 `desktop_licenses.buyer_id` 与 `desktop_usage_reservations` 的原图字节、压缩后字节、软件版本字段，兼容旧客户端缺少这些指标的历史回传。

### Tests

- **回归验证**：桌面端 35 项测试、Gitee 同步 5 项测试、Rust 31 项测试、桌面授权后端 18 项测试、桌面端与网页端生产构建通过。

## [1.14.47] - 2026-07-27

### Fixed

- **Windows 启动窗口**：正式安装包不再额外打开黑色控制台窗口，保留调试构建的控制台输出能力。

### Changed

- **桌面端授权 Token**：激活输入同时兼容完整 HTTPS Auth Link 与直接粘贴 Token；授权创建与读取接口统一只返回 `token`，响应禁止缓存，后台复制内容改为授权 Token。
- **桌面端产品约束**：明确 TinyPNG 近似无损优化不以压缩倍率作为任务成败、策略或发布验收标准，优先保证压缩成功、原图保护与结果可用。
- **桌面端版本**：TinyPNG 压缩助手升级至 `0.2.12`。

### Tests

- **授权与构建回归**：桌面端 35 项测试、Rust 30 项测试、桌面端生产构建与网页端生产构建通过。
