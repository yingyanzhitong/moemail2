...
- **桌面授权到期日**：30 天及其整数倍授权改为按自然月计算，7 月 27 日下发的 30 天授权将在 8 月 27 日同一时刻到期，不再固定按 30 × 24 小时提前结束。
- **历史有效授权补正**：发布迁移会修正仍有效授权的当前及排队续费周期，并保证相邻周期连续衔接；非 30 天整数倍的自定义周期仍按实际天数计算。

### Tests

- **授权周期回归**：验证 30 天、90 天、月末日期、续费排队和生产数据迁移场景；桌面授权后端 21 项测试与网页端生产构建通过。
