...
function CalibrationBars({ item }: { item: QueueItem }) {
  ...
  return (
    <div className="mt-3 grid grid-cols-[88px_1fr] items-center gap-x-3 gap-y-1.5 text-[10px] text-[#778196]">
      ...
      <span className="font-mono">{item.compressedSize ? `压缩 ${formatBytes(item.compressedSize)}` : '等待压缩'}</span>
      ...
      {items.length === 0 ? (
        ...
        <div className="queue-scroll max-h-[calc(100vh-265px)] min-h-[290px] overflow-y-auto p-2">
          {items.map((item, index) => (
            <article key={item.id} className="group grid grid-cols-[44px_minmax(0,1fr)_auto] gap-3 rounded-[10px] border border-transparent px-2.5 py-3 hover:border-[#D6DDE8] hover:bg-[#FAFBFD]">
