...
同一次构建还会生成 macOS `.app.tar.gz`、Windows `.exe.sig` 及 Gitee `release/latest.json`。两套 macOS 与 Windows 的安装包、更新包、签名及 `latest.json` 会完整同步到 Gitee Release；只有全部 9 个非源码资产就绪后，才更新客户端读取的 `release/latest.json`。应用启动和每小时会检查该清单，左上角“检查更新”可手动检查；下载后由 Tauri 使用内置公钥验证签名再安装并重启。签名私钥只保存在 GitHub Actions Secret，不进入仓库或安装包。
