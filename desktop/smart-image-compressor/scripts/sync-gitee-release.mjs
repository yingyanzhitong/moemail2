...
async function findReleaseFiles(directory) {
  ...
  for (const entry of entries) {
    ...
    } else if (entry.isFile() && isReleaseBinaryAssetName(entry.name)) {
...
async function uploadAsset(releaseId, releaseTag, file, name, attempt, maxAttempts) {
  ...
  try {
    await execFileAsync('curl', [
      ...
      '--max-time', String(config.uploadTimeoutSeconds), '--request', 'POST', '--header', 'Expect:',
      '--form', `file=@${file};filename=${name};type=application/octet-stream`, url,
